...
const OnlineLobby = ({ onBack, onGameStart }) => {
     ...
     const createRoom = async () => {
         ...
         if (!user) { alert("ログインできていません。コンソールを確認してください。"); return setError("Login failed"); }
         ...
         try {
             ...
             const unsub = onSnapshot(doc(db, "battles", newRoomId), (doc) => { const data = doc.data(); if (data && data.guestId) { unsub(); onGameStart({ roomId: newRoomId, role: 'host' }); } });
         } catch (e) { console.error("Room Create Error:", e); alert("部屋の作成に失敗しました。\nFirebaseのセキュリティルールを確認してください。\n" + e.message); }
     };
     const joinRoom = async () => {
         ...
         if (roomSnap.exists() && roomSnap.data().status === 'waiting') { await updateDoc(roomRef, { guestId: user.uid, status: 'matched' }); onGameStart({ roomId: inputRoomId, role: 'guest' }); } else { setError("Room not found or full"); }
     };
    return (
